...
The following icons dignify changes to slide from the previous iteration.
...
| Optional Message type and direction | Exception & Investigation Case Assignee | Payment Clearing and Settlement (pacs) |
| Focal Message type and direction | Statement Account Owner | Cash Management Reporting (camt) |
| pacs Debtor | Statement Account Servicer | Cash Management Exception & Investigations (camt) |
| pacs Creditor | Statement Authorised Party | Focus message |
| pacs.008 Ultimate Debtor | Shortcut to other part of document | Element Hierarchy Path |
| pacs.008 Ultimate Creditor | Extra attention is needed | Nested Element |
| Initiating party on behalf of the Debtor | Legacy FIN MT message | Element Choice |
| Agent | gpi Tracker | Nested Element involving a choice |
| Agent processing legacy format during a coexistence period | | New slide since last iteration |
| Market Infrastructure | | Slide updated since last iteration |
...
<!-- ELEMENT 10 -->
introduced in ISO 20022 | :AX PIN MT format equivalent
...
* Previous Instructing Agents (:72/INS/)
* Reimbursement Agents (:53a :54a :55a)
* Intermediary Agents (:56a :72/INT/)
* Debtor Agent (:52a)
...
`<Document>` ... $\rightarrow$ ISO 20022 Message
**(Combined)** $\rightarrow$ Business Message
...
An empty XML element is an element that contains no data content and therefore is described as empty. Although this makes little business sense, this is a known feature of XML. It typically occurs where a nested element beneath a parent element allows for various element options but does not either enforce the use of an element with a minimum of 1 occurrence or does not have a rule defining the presence of at one of the nested element.

A common example of this is in pavment message is Financial Institution.
...
<!-- ELEMENT 14 -->
MyStandards describes the element's context by its path whereby each element is divided by a forward slash (/) For example the pacs.008 Town Name within the Debtor Postal Address is described as /Document/FitToFlCstmrCdtTrf/CdtTrfTxInf/Dbtr/PstlAdr/TwnNm

Within the CBPR+ a similar Path principal is often used to visualize an element being explained, where the name is expanded rather than describing the element in an XML naming convention. For example to describe the pacs.008 Payment Identification, the following is displayed on the bottom right hand side of the page to explain Payment Identification is an element below Credit Transfer Transaction Information.
...
| :--- | :--- | :--- |
| CBPR+ user handbook | Explains how CBPR+ Usage Guidelines support common business scenarios in cross-border payment and reporting processes. | Download > |
| Data integrity market practice guidance | Defines recommended market practices to report truncated or missing payment information using the MT messaging standard. | Download > |
| CBPR+ and HVPS+ alignment | Lists the similarities and differences between the two market practice sets. | Download > |
| Derived data mapping guidance | Provides guidance on how to populate information from one messaging standard to the other subsequent message exchange. | Download > |
| MT / MX equivalence table | Documents the CBPR+ Usage Guidelines supported on SwiftNet FINplus service in comparison with the equivalent MT message. | Download > |
| Lessons learnt since go-live | This document intends to provide guidance to the community regarding observations that may be identified before or after the migration. | Download > |

### Samples library

| Title | Description | Action |
| :--- | :--- | :--- |
| CBPR+ sample messages | Sample messages sets to test the use cases described in the CBPR+ user handbook. | Learn more > |
| In-flow translation service sample messages | Sample test messages for the in-flow translation service. | Learn more > |
...
<!-- ELEMENT 16 -->
**Cross Border Payments and Reporting Plus** | PUBLISHED | MyStandards 🏠

---

# CBPRPlus-pacs.008.001.08_FitToFICustomerCreditTransfer 🔒
...
...Credit Transfer V08 (pacs.008.001.08)
...
| Field | Value |
| :--- | :--- |
| Publisher | Cross Border Payments and Reporting Plus (CBPR+) |
| Technical version | 1 |
| Version | V2.0 FOR IMPLEMENTATION |
| Status | Final |
| Standard | pacs.008.001.08 |
| Format | MX |
| Usage identifier | swift.cbprplus.01 |
| Documents | For direct download + add |
| Principles | **Description:** <br> 1A. AGENTS IDENTIFICATION - RECOMMENDATION <br> - If BIC is present, then (Name and Postal Address) is NOT allowed (ClearingSystemMemberIdentification and LEI may complement) – However, in case of conflicting information, the BIC will always take precedence. <br> - If BIC is absent, (Name and Postal Address or ClearingSystemMemberIdentification must be present and both are allowed together. <br> Note: "Instructing Instructed Agents" must be identified with a BIC – Clearing System-Members Identification and LEI are optional. <br> 1B. DEBTOR/CREDITOR present - PARTY IDENTIFICATION - RECOMMENDATION <br> - If BIC is present, then (Name and Postal Address) is NOT allowed (other elements remain optional) – However, in case of conflicting information, AnyBIC will always take precedence. |
...
**Formal Rules** which are validated on the network, these are identified by the word **Rule** appended to the rule description. For example: ⚠️ **Rule “CBPR_Party_Name_Any_BIC_FormalRule”**

**Textual Rules** which are not validated on the network, these are identified by the word **TextualRule** appended to the rule description. For example: ⚠️ **Rule “CBPR_Agent_Option_1_TextualRule”**

**Guideline Rules** which provide recommended best practice, these are identified by the word **Guideline** appended to the rule description. For example: ⚠️ **Rule “CBPR_Purpose_Guideline”**
...
| 1-10 chars | 1 char | 1-10 chars | 1 char | 1-10 chars | 1 char | 2 chars |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **A** | . | **B** | . | **C, D,...** | . | **E** |
...
| Element | Min | Max |
| :--- | :---: | :---: |
| Return Reason Information | 0 | * |
| ↳ Originator | 0 | 1 |
| ↳ ↳ Reason | 0 | 1 |
| ↳ ↳ ↳ Code | 1 | 1 |
| ↳ ↳ ↳ Proprietary | 1 | 1 |

**Type**: ExternalReturnReason1Code (base type: string)
- minLength: 1
- maxLength: 4
...
<!-- ELEMENT 20 -->
| All SWIFT ISO MX message elements (fields) which are defined (by data Type) as text are restricted to FIN X Characters:<br><br>a-z A-Z 0-9 / - ? : ( ) . , ' + . | Special characters are additionally allowed in:<br><br>• All party (agents and non-agents) Name and Address elements.<br>• The Related Remittance Information element<br>• The Remittance Information (structured & unstructured) element<br>• The Email Address where included as part of a Proxy elements<br>• City of Birth and Province of Birth elements nested in Private Identification<br><br>List of special characters:<br>`!#%&*=^_{|}~<>[ ]\()@{` | Currencies in the payments should be expressed in ISO Currency Codes only (3-Characters, e.g. EUR) | Translation of any special character: `!#%&*=^_{|}~";"@[]$><` into MT messages will be represented by a . (Full Stop)<br><br> $ ! / % <br> < # ] * <br> > & [ = |
| :--- | :--- | :--- | :--- |
...
- BICFI
- Clearing System Member Id $\rightarrow$ Clearing System Id
- LEI
- Name
...
| | | |
| :--- | :--- | :--- |
| **Business Application Header** | Message Definition Identifier | pacs.009.001.08 |
| **Business Document** | <Document "pacs.009.001.08"> | Group Header |
...
| :--- | :--- |
| pain.001.001.09 | swft.cbprplus.02 |
| pain.002.001.10 | swft.cbprplus.02 |
| pain.008.001.03 | swft.cbprplus.01 |
| pacs.002.001.10 | swft.cbprplus.02 |
| pacs.003.001.08 | swft.cbprplus.01 |
| pacs.004.001.09 | swft.cbprplus.02 |
| pacs.008.001.08 | swft.cbprplus.02 |
| pacs.008.001.08 (STP/STP EUJ) | swft.cbprplus.stp.02 |
| pacs.009.001.08 (advice) | swft.cbprplus.adv.02 |
| pacs.009.001.08 (core) | swft.cbprplus.02 |
| pacs.009.001.08 (cov) | swft.cbprplus.cov.02 |
| pacs.010.001.03 | swft.cbprplus.02 |
| pacs.010.001.03 (Margin Collection) | swft.cbprplus.col.01 |
| camt.029.001.09 | swft.cbprplus.02 |
| camt.052.001.08 | swft.cbprplus.02 |
| camt.053.001.08 | swft.cbprplus.02 |
| camt.054.001.08 | swft.cbprplus.02 |
| camt.055.001.08 | swft.cbprplus.01 |
| camt.056.001.08 | swft.cbprplus.02 |

| Message Definition Identifier | Business Service |
| :--- | :--- |
| camt.058.001.08 | swft.cbprplus.01 |
| camt.060.001.05 | swft.cbprplus.02 |
| camt.107.001.01 | swft.cbprplus.01 |
| camt.108.001.01 | swft.cbprplus.01 |
| camt.109.001.01 | swft.cbprplus.01 |
...
| :--- | :--- |
| **Group Header**<br>➢ Message Identification<br>➢ Initiating Party – where different from Debtor<br>➢ Forwarding Agent | **Sequence A – General Information:**<br>➢ Sender's Reference (Field 20)<br>➢ Instructing Party (Field 50 C or L) |
| **Payment Information**<br>➢ Payment Information Identification<br>➢ Requested Execution Date<br>➢ Debtor<br>➢ Debtor Agent | Message *Sender* in a 'relay' scenario<br>**Sequence A – General Information:**<br>➢ Customer Specified Reference (Field 21R)<br>➢ Requested Execution Date (Field 30)<br>➢ Ordering Customer (Field 50 F, G or H)*<br>➢ Account Servicing Institution (Field 52 A or C)* |
| **Credit Transfer Transaction Information**<br>➢ Payment Identification<br>➢ Amount<br>➢ Charge Bearer<br>➢ Creditor Agent<br>➢ Creditor | **Sequence B – Transaction Details:**<br>➢ Transaction Reference (Field 21)<br>➢ Currency/Transaction Amount (Field 32B)<br>➢ Details of Charges (Field 71A)<br>➢ Account With Institution (Field 57 A, C or D)<br>➢ Beneficiary (Field 59 no letter, A or F) |
...
<!-- ELEMENT 43 -->
Debtor | Initiating Party* | Forwarding Agent | Debtor Agent | Creditor Agent | Creditor
---|---|---|---|---|---
| $\rightarrow$ pain.001** | $\rightarrow$ FINplus pain.001 | $\rightarrow$ pacs.008 | | $\rightarrow$ camt.053 |
| $\leftarrow$ pain.002 | $\leftarrow$ FINplus pain.002 | $\leftarrow$ pacs.002 | | |
...
**Message Flows:**
*   Initiating Party $\rightarrow$ Debtor Agent: `FINplus pain.001`
*   Debtor Agent $\rightarrow$ Initiating Party: `FINplus pain.002`
*   Debtor Agent $\rightarrow$ Creditor Agent: `pacs.008`
*   Creditor Agent $\rightarrow$ Debtor Agent: `pacs.002`
*   Creditor Agent $\rightarrow$ Creditor: `camt.053`
...
<!-- ELEMENT 45 -->
![Diagram showing payment message flow between Debtor, Initiating Party (Bank I), Debtor Agent (Bank A), Bank B, Creditor Agent (Bank C), and Creditor]

**Message Flow:**
*   Initiating Party $\xrightarrow{\text{Interbank pain.001}}$ Debtor Agent
*   Debtor Agent $\xrightarrow{\text{Interbank pain.002}}$ Initiating Party
*   Debtor Agent $\xrightarrow{\text{pacs.008}}$ Bank B
*   Bank B $\xrightarrow{\text{pacs.002}}$ Debtor Agent
*   Bank B $\xrightarrow{\text{pacs.008}}$ Creditor Agent
*   Creditor Agent $\xrightarrow{\text{pacs.002}}$ Bank B
*   Creditor Agent $\xrightarrow{\text{camt.053}}$ Creditor
...
Group Header $\rightarrow$ Number of Transactions
...
The **Initiating Party** can be either the **Debtor** or an Authorised Party who initiates payment transactions on behalf of the **Debtor**. The Initiating Party can be, for example, the Head Office which has the authority to debit the account of its subsidiary. In the centralization model, the *Initiating Party* can also be a payment factory or shared service center or third-party agent, which has authority to send the message on behalf of the *Debtor*.
...
### Initiating Party
- **Name**: Mandatory **Name** where Postal Address is provided.
- **Postal Address**: Nested element capturing **structured** Postal Address including at least Town Name and Country if used.
- **Identification**: Nested element capturing the various types of identifiers, e.g., BIC, LEI etc. **BIC** is mandatory for an Authorised Party initiation and FI payment initiation.
- **Country of Residence**: Optional element to capture the Initiating Party's ISO country code of residence.
- **Contact Details**
...
(Structured) (Unstructured) $\rightarrow$ [pain.001] $\rightarrow$ (Unstructure) Many domestic proprietary payments

(Structured) (Unstructured) CBPR+ payments
...
Min 0 – Max 1
A choice of imbedded codes representing the urgency considered by the instructing party. This point-to-point information may be used by the instructed party to differentiate the processes
...
Component Information - Requested Execution Date
...
*   **Name**: Mandatory Name (where a BIC identifier is not provided) by which the party is known.
...
*   **Country of Residence**: Optional element to capture the Debtor's ISO country code of residence.
...
| Cardinality | Element | Description |
| :--- | :--- | :--- |
...
### MT 101
| 71A: Details of Charges | Code | Description |
| :--- | :--- | :--- |
| | BEN | Beneficiary |
...
| Draft | DRFT | A guaranteed bank cheque with a future value date (do not pay before], which in commercial terms is a 'negotiatable instrument': the beneficiary can receive early payment from any bank under subtraction of a discount. The ordering customer's account is debited on value date. |
...
Credit Transfer Transaction Information ✔️
...
*   **Name**: Mandatory Name (where a BIC identifier is not provided) by which the party is known
*   **Postal Address**: Nested element capturing either structured or unstructured *Creditor* address details. Note: Structured address is strongly recommended with mandatory Town Name and Country
*   **Identification**: Nested element capturing the various types of identifiers for the party e.g. BIC, LEI etc.
...
Credit Transfer Transaction Information $\rightarrow$
...
Credit Transfer Transaction Information > Regulatory Reporting > Debit Credit Reporting Indicator > Authority
...
| Element | Constraint |
| :--- | :--- |
| Creditor | Min 0 – Max 1 |
| Debtor | Min 0 – Max 1 |
| Administration Zone | Min 0 – Max 1 |
| Reference Number | Min 0 – Max 1 |
| Method | Min 0 – Max 1 |
| Total Taxable Base Amount | Min 0 – Max 1 |
| Total Tax Amount | Min 0 – Max 1 |
| Date | Min 0 – Max 1 |
...
Recommend to refer to CGI-MP Document Centre for Country | Credit Transfer Transaction Information | Remittance Information | Structured
...
[Diagram: Structured $\rightarrow$ Referred Document Information, Referred Document Amount, Creditor Reference Information, Invoice, Invoice, Tax Remittance, Garnishment Remittance, Additional Remittance Information]
...
| element names | xml tag | business information |
| :--- | :--- | :--- |
| **Structured** | `<Strd>` | |
| Reference Document Information | `<RfrdDocInf>` | |
| Type | `<Tp>` | |
| Code Or Proprietary | `<CdOrPritry>` | |
| Code | `<Cd>` | CINV |
| Number | `<Nb>` | A0000101 |
...
Credit Transfer Transaction Information $\rightarrow$ Remittance Information
...
Credit Transfer Transaction Information > Remittance Information
...
**Message Flow:**
* Initiating Party $\rightarrow$ Forwarding Agent (F): `pain.001`, `pain.002`
* Forwarding Agent (F) $\rightarrow$ Debtor Agent (A): `Interbank pain.001`, `Interbank pain.002`
* Debtor Agent (A) $\rightarrow$ Creditor Agent (B): `pacs.008`
* Creditor Agent (B) $\rightarrow$ Final Party: `camt.053`
...
* **3**: Debtor Agent (A) debits the account of Debtor and initiates a credit transfer by forwarding a local credit transfer message pacs.008
...
* **4**: Creditor Agent (B) processes the payment and sends the statement to Creditor
...
**Diagram Flow:**
- 1 $\xrightarrow{\text{pain.001}}$ F $\xrightarrow{\text{Interbank pain.001}}$ A $\xrightarrow{\text{pacs.008}}$ B $\xrightarrow{\text{camt.053}}$ [End Entity]
- [End Entity] $\xleftarrow{\text{pain.002}}$ F $\xleftarrow{\text{Interbank pain.002}}$ A
...
*   **3a**: Debtor Agent (A) provides a status update ACSP (accepted settlement in progress) to the
*   **3b**: Forwarding Agent (F) relays the status ACSP (accepted settlement in progress) to the Initiating Party, based upon a bilateral agreement
*   **4**: Creditor Agent (B) processes the payment and notifies Creditor of a successful sweep through the
...
**Diagram Flow:**
- Initiating Party $\rightarrow$ (pain.001 / pain.002) $\rightarrow$ Forwarding Agent (F) $\rightarrow$ (Interbank pain.001 / Interbank pain.002) $\rightarrow$ Debtor Agent (A) $\rightarrow$ (pacs.008) $\rightarrow$ Payment Market Infrastructure $\rightarrow$ (pacs.008) $\rightarrow$ Creditor Agent (B) $\rightarrow$ Recipient

**Steps:**
- **1**: Initiating Party sends a payment instruction to the Forwarding Agent
- **2**: Forwarding Agent (F) forwards the payment instruction to the Debtor Agent (A)
- **3**: Debtor Agent (A) debits the account of Debtor and initiates a credit transfer by forwarding a local credit transfer message pacs.008 to a Payment Market Infrastructure (PMI)
- **3a**: Debtor Agent (A) provides a status update ACSP (accepted settlement in progress) to the Forwarding Agent (F), based upon a bilateral
- **3b**: Forwarding Agent (F) relays the status ACSP (accepted settlement in progress) to the Initiating Party, based upon a bilateral agreement
...
**Process Flow:**
Debtor $\rightarrow$ DA (Debit Authorisation) $\rightarrow$ Agent I $\xrightarrow{2}$ Debtor Agent (A) $\xrightarrow{3}$ Creditor Agent (B) $\xrightarrow{4}$ Creditor
(Return path: Debtor Agent (A) $\xrightarrow{3a}$ Agent I)

**Messages:**
*   **Step 2:** Interbank pain.001 / Interbank pain.002
*   **Step 3:** pacs.008 / pacs.002
*   **Step 4:** camt.053
